...
export default function PartnersTable({ byPartnerMonth }) {
  if (!byPartnerMonth?.length) return null;

  // Aggregate across filtered period
  const map = {};
  ...
  const partners = Object.values(map).map(p => {
    ...
    return {
      ...
      gmv:     p.gmv,
      orders:  p.orders,
      aov:     p.orders > 0 ? p.gmv / p.orders : 0,
      momGMV,
    };
            ...
            {partners.map((p, i) => {
              ...
              return (
                <tr key={`${p.country}__${p.partner}`} className="hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
                  ...
                  <td className="px-5 py-3 font-medium text-gray-900 dark:text-white max-w-[220px] truncate">
                    {p.partner}
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <div className="w-14 bg-gray-100 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.min(gmvPct, 100)}%` }}
                        />
                      </div>
                  ...
                  <td className="px-5 py-3 text-right">
                    {p.momGMV !== null ? (
                      <span className={`text-xs font-bold ${trendColor(p.momGMV)}`}>
                        {fmtPct(p.momGMV)}
                      </span>
                    ) : (
